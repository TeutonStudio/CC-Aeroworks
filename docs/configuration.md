...
Die Taste `key.cc_aeroworks.combined_lever` ist standardmäßig auf `K` gelegt. Sie erscheint in den Minecraft-Steuerungsoptionen als `Kombiniert (Lever, Maus Y)` in der Kategorie `CC-Aeroworks` und kann dort frei geändert werden.
