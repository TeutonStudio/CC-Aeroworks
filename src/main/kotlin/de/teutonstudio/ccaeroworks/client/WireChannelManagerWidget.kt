...
internal class WireChannelManagerWidget(
    ...
    fun selectedChannel(): WireChannelView? = WireChannelSnapshotState.get().wire.channels.firstOrNull { it.id == selectedWireId }
    fun selectedTargetId(): String? = selectedTargetId
    fun selectedGroupId(): UUID? = selectedGroup
    fun selectedBindingAlias(): String? = selectedBinding
    ...
    override fun renderWidget(graphics: GuiGraphics, mouseX: Int, mouseY: Int, partialTick: Float) {
        ...
        if (selectedWireId != null && snapshot.wire.channels.none { it.id == selectedWireId }) selectedWireId = null
        if (selectedGroup != null && snapshot.userGroups.none { it.id == selectedGroup }) { selectedGroup = null; selectedBinding = null }
    ...
    override fun mouseClicked(mouseX: Double, mouseY: Double, button: Int): Boolean {
        ...
        when (val row = rows().getOrNull(scrollIndex + rowIndex)) {
            ...
            is ChannelRow.Control -> { selectedTargetId = row.channel.id; selectedWireId = null; selectedBinding = null }
            is ChannelRow.Wire -> { selectedWireId = row.channel.id; selectedTargetId = "wire:${row.channel.id}"; selectedBinding = null }
            is ChannelRow.UserGroup -> { selectedGroup = row.group.id; selectedBinding = null; toggle("user:${row.group.id}") }
            is ChannelRow.Binding -> { selectedGroup = row.groupId; selectedBinding = row.binding.alias; selectedTargetId = row.binding.targetId; selectedWireId = null }
            else -> Unit
        }
        return true
    }
    ...
    private fun renderWire(graphics: GuiGraphics, channel: WireChannelView, rowY: Int, hovered: Boolean) {
        val selected = channel.id == selectedWireId; if (selected || hovered) graphics.fill(x + 2, rowY + 1, x + width - 2, rowY + ROW_HEIGHT - 1, if (selected) 0xFF303030.toInt() else 0xFF202020.toInt())
    ...
    private fun renderUserGroup(graphics: GuiGraphics, group: UserChannelGroupView, rowY: Int, hovered: Boolean) {
        val selected = selectedGroup == group.id && selectedBinding == null; if (selected || hovered) graphics.fill(x + 2, rowY + 1, x + width - 2, rowY + ROW_HEIGHT - 1, if (selected) 0xFF303030.toInt() else 0xFF242424.toInt())
    ...
    private fun renderBinding(graphics: GuiGraphics, groupId: UUID, binding: UserChannelBindingView, rowY: Int, hovered: Boolean) {
        val selected = selectedGroup == groupId && selectedBinding == binding.alias; if (selected || hovered) graphics.fill(x + 2, rowY + 1, x + width - 2, rowY + ROW_HEIGHT - 1, if (selected) 0xFF303030.toInt() else 0xFF1C1C1C.toInt())
