...
local okSources, reactiveSources = pcall(require, "cc_aeroworks.reactive_sources")
if not okNative or not okUi then
    report("CC-Aeroworks display runtime unavailable: " .. tostring(okNative and ui or native))
    return
end
if not okSources then
    report("CC-Aeroworks reactive sources unavailable: " .. tostring(reactiveSources))
end
...
-- branch is merged with a master containing that compatibility autorun.
...
os.pullEventRaw = function(filter)
    while true do
        ...
        if okSources and type(reactiveSources.handleEvent) == "function" then
            local ok, err = pcall(reactiveSources.handleEvent, event)
            if not ok then report("CC-Aeroworks reactive source observer: " .. tostring(err)) end
        end
