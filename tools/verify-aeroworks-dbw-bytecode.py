...
def to_class_name(entry: str) -> str:
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="cc-aeroworks-aeroworks-dbw-") as directory:
        ...
        with zipfile.ZipFile(jar) as archive:
            ...
            compat_names = [
                ...
                and "compat/drivebywire/" in entry.lower()
                and "$" not in entry
            ]
            require(compat_names, "Aeroworks release has no Drive By Wire compat classes")
        ...
        require("selectedSource" in client_combined and "currentChannel" in client_combined,
                "Aeroworks DBW client integration no longer owns selected source/channel state")
        require("ConsoleWireChannels.nextChannel" in client_combined,
                "Aeroworks DBW client integration no longer delegates modular cycling to ConsoleWireChannels")

        compat_outputs = {name: javap(jar, name) for name in compat_names}
        publishers = {
            name: output for name, output in compat_outputs.items()
            if "WireNetworkManager" in output or "trySetSignalAt" in output
        }
        require(publishers,
                "Aeroworks Drive By Wire compat classes expose no WireNetworkManager publication path")
        publish_combined = "\n".join(publishers.values())
        require("trySetSignalAt" in publish_combined,
                "Aeroworks DBW publication no longer uses trySetSignalAt; update CC-Aeroworks display isolation")
        print("Aeroworks DBW publishers:", ", ".join(sorted(publishers)))
    ...
    print(f"Validated exact Aeroworks {VERSION}: directional channel identity and DBW publication contract are stable.")
    return 0
